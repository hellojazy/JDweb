...
程序还包含商品管理、生成历史、表格预览、下载、库存不足预警、滞销分仓预警和 7 日销售排行图表。
...
```text
7日销售额 = 近7日出库商品件数 * 全国采购价
```

图表中的占比为该行销售额占全部统计销售额的比例。产品排行优先使用配送中心为 `全国` 的 SKU 汇总行，避免把全国汇总和分仓数据重复计算；没有 `全国` 汇总行的 SKU，则按分仓数据汇总。站点排行只统计分仓站点，不统计 `全国` 和 B 仓。
